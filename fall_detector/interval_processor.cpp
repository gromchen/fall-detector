...
void IntervalProcessor::IncludeObject(FrameData frameData)
{
    ...
    if(number_of_milliseconds >= 1000)
    {
        ...
        for(unsigned int iEllipse = 0; iEllipse < mFrameDataCollection.size(); iEllipse++)
        {
        ...
        {
            double average_of_angles = sum_of_angles/number_of_frames_with_found_object;
            double average_of_ratios = sum_of_ratios/number_of_frames_with_found_object;
            double mean_square_sum_of_angles = 0;
            double mean_square_sum_of_ratios = 0;

            for(unsigned int iFoundObject = 0; iFoundObject < angles.size(); iFoundObject++)
            {
                mean_square_sum_of_angles += pow(angles[iFoundObject] - average_of_angles, 2);
                mean_square_sum_of_ratios += pow(ratios[iFoundObject] - average_of_ratios, 2);
            }

            standard_deviation_of_orientation = sqrt(mean_square_sum_of_angles / number_of_frames_with_found_object);
            standard_deviatioin_of_ratio = sqrt(mean_square_sum_of_ratios / number_of_frames_with_found_object);
        }
        else
        {
